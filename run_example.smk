...
import natsort
import re
files = glob_wildcards(os.path.join("1_data", "{subfolder_filename}.tif"))
both = glob_wildcards(os.path.join("1_data", "{subfolder}", "{filename}.tif"))
both.subfolder
both.filename

rule all:
    input:
#        expand(os.path.join("2_segmentation", "{subfolder_filename}.tif"), subfolder_filename = files.subfolder_filename)
        expand(os.path.join("4b_time_averaged_cell_morphodynamics","{subfolder}","cell_data.csv"), subfolder = both.subfolder),
        expand(os.path.join("5_tracking_images_outlines","{subfolder_filename}.tif"), subfolder_filename = files.subfolder_filename)

####################################################################################################
# Preprocessing

def get_stabilization_files_list_from_subfolder(wildcards):
    this_original_sub = os.path.join("1_data", wildcards.subfolder)
    this_root_sub = os.path.join("1_data", wildcards.subfolder)
...
#    output:
#        os.path.join("1b_stabilize","{subfolder}","{filename}.tif")
#        [os.path.join("1b_stabilize","{subfolder}","{filename}.txt").format(filename=filename) for filename in [each2 for each2 in os.listdir(os.path.join("1b_stabilize","{subfolder}","")) if not each2.startswith('.')]]
#        os.path.join("1b_stabilize","{subfolder}","image_stack.npy")
#    script:
...
#        os.path.join("1_data","{subfolder_filename}.tif")
#    output:
#        os.path.join("1c_stabilize","{subfolder_filename}.tif")
#    script:
#        os.path.join("scripts","run_group_back_to_individuals.py")
...
#    this_root_sub = os.path.abspath(os.path.join("..","..","2024-03-14_snakemake_develop_tracking","MorphoDynamicsPipe","2_segmentation",""))
    this_root_sub = os.path.abspath(os.path.join("..","..","2024-07-30_new_lifs_batch_nucs_slightly_broader_dataset","MorphoDynamicsPipe","1_data",""))
...
#        os.path.join("1_data","{subfolder_filename}.tif")
#    output:
#        os.path.join("2_segmentation","{subfolder_filename}.tif")
#    retries: 10
#    conda:
...
##        "stardist0"
#    script:
#        os.path.join("scripts","run_stardist.py")
...
##        "microsam0"
#    script:
#        os.path.join("scripts","run_microsam.py")
...
##        "cellpose2"
    script:
        os.path.join("scripts", "run_cellpose_nucs.py")
...
#    retries: 10
#    script:
#        os.path.join("scripts","run_cellpose_celltracker_with_nucs.py")
...
def get_segmentation_files_list_from_subfolder(wildcards):
    this_original_sub = os.path.join("1_data", wildcards.subfolder)
    this_root_sub = os.path.join("2_segmentation", wildcards.subfolder)
...
def get_tracking_info_from_subfolderfilename_nuclei_version(wildcards):
    ...
    tracking_info_base_path = os.path.abspath(os.path.join('..', '..', '2024-07-30_new_lifs_batch_nucs_slightly_broader_dataset', 'MorphoDynamicsPipe', '3a_tracking_info'))
...
rule track_with_btrack:
    ...
    output:
        os.path.join("3a_tracking_info", "{subfolder}", "track_info.npy")
    script:
        os.path.join("scripts", "run_btrack_to_info.py")
...
rule convert_btrack_info_to_images:
    ...
    output:
        os.path.join("3b_tracking_images", "{subfolder_filename}.tif")
    script:
        os.path.join("scripts", "convert_btrack_info_to_images.py")
...
rule filter_cells_after_tracking:
    input:
        ...
        get_tracking_info_from_subfolderfilename,
    output:
        os.path.join("3c_tracking_images_filtered", "{subfolder_filename}.tif")
    params:
        number_of_frames_to_remove_object = 0
    script:
        os.path.join("scripts", "filter_short_lived_cells_after_tracking.py")
...
def get_segmentation_relabeled_files_list_from_subfolder(wildcards):
    this_original_sub = os.path.join("1_data", wildcards.subfolder)
#    this_root_sub = os.path.join("3b_tracking_images", wildcards.subfolder)
    this_root_sub = os.path.join("3c_tracking_images_filtered", wildcards.subfolder)
...
rule extract_instantaneous_cell_morphodynamics:
    ...
    output:
        os.path.join("4a_instantaneous_cell_morphodynamics", "{subfolder}", "cell_data.csv")
    retries: 2
    script:
        os.path.join("scripts", "extract_instantaneous_cell_morphodynamics.py")
...
rule get_maximum_common_time:
    ...
    script:
        os.path.join("scripts", "get_maximum_common_time.py")
...
rule extract_time_averaged_cell_morphodynamics:
    input:
        ...
        get_segmentation_relabeled_files_list_from_subfolder,
    output:
        os.path.join("4b_time_averaged_cell_morphodynamics", "{subfolder}", "cell_data.csv")
    script:
        os.path.join("scripts","extract_time_averaged_cell_morphodynamics.py")
...
#        os.path.join("3b_tracking_images","{subfolder_filename}.tif")
        os.path.join("3c_tracking_images_filtered", "{subfolder_filename}.tif")
    output:
        os.path.join('5_tracking_images_outlines', '{subfolder_filename}.tif')
    script:
        os.path.join("scripts", "convert_to_label_outlines.py")
